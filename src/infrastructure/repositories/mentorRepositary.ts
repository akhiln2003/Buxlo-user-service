...
export class MentorRepository implements ImentorRepository {
  ...
  }async updateMentorProfile(userId: string , query:IupdateData): Promise<Mentor | null> {
    try {            
...
async updateMentorProfileData(userId: string, data: { name?: string; avatar?: string }): Promise<Mentor | null> {
    try {
